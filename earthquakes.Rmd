...
## 8. Visualize the distribution of magnitude between earthquake types

How does the distribution of magnitude vary between earthquake types? 
...
- Which observations had depth errors larger than 40? List those observations showing the status, depth, depthError, place, and mag columns.
...
The elapsed time between the two seismic events is the first time minus the second time. Since `read_csv()` automatically formatted the date-times for us, we can perform subtraction to get elapsed time.
...
The `as.duration` function from lubridate creates a numeric time measurement that prints both seconds and approximate minutes.

```{r}
library(lubridate)
d <- d %>% 
  mutate(elapsed_time = as.duration(time - lead(time)))
d %>% 
...
- What is the median elapsed time? In other words, about how often is a seismic event recorded by the USGS?
...
What if we want to look at data just for the United States? We need to identify those records that are occurred in a US state. One way to do that is with a _regular expression_. Below we build a regular expression using the built-in `state.name` vector that comes with R. But we also append "CA" to it since our earthquake data always lists California as "CA". 

The regular expression is simply all 50 US states in a single set of parentheses separated by a vertical bar (|). This allows us to check for multiple instances of text at the end of a string. For example the regular expression "(car|bike|boat)" allows us to look for the words "car", "bike" or "boat" at the same time. 
