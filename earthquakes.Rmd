...
_In this project you will import, visualize and summarize data on earthquakes by completing a series of tasks._
...
Using the URL above, import the data as a data frame object. Tip: use the `read_csv()` function in the readr package. The base R `read.csv` works just fine, but the `read_csv` function automatically formats the dates for us. Also recall that `read_csv()` works with URLs.
...
How is the depth of an earthquake associated with its magnitude? Create a visualization to investigate. Comment on this relationship (one sentence will do.)
...
## 4. Investigate the various seismic events
...
The "depth" variable measures the depth of an earthquake in kilometers. The "depthError" column reports the uncertainty of the reported depth. Is "depthError" associated with "depth"? Create a plot to investigate and visualize the relationship.

Are there influential "depthError" observations that suppress or amplify this relationship? Tip: it may help to filter the data for "depthError" less than 30.
...
## 7. Visualize the relationship between magError and magType
...
## 8. Investigate location of earthquakes
...
## 9. Do seismic events in some locations tend to have larger magError?
...
## 10. Investigate elapsed time between earthquakes

The data is ordered in descending order, with the _most recent earthquake first_. Below we calculate elapsed time between earthquakes. For example, the first two rows have the following times:
